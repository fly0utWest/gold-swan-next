import {
  Magnifer,
  Graph,
  Link,
  User,
  Target,
  Volume,
  Global,
  Cursor,
  Mailbox,
  LinkRoundAngle,
  Chart,
} from "solar-icon-set";

export const seoServices = [
  {
    Icon: Magnifer,
    title: "seo.features.visibility.title",
    description: "seo.features.visibility.description",
  },
  {
    Icon: Graph,
    title: "seo.features.keywordOptimization.title",
    description:
      "seo.features.keywordOptimization.description",
  },
  {
    Icon: Link,
    title: "seo.features.backlinks.title",
    description: "seo.features.backlinks.description",
  },
];

export const contentMarketingServices = [
  {
    Icon: User,
    title: "contentMarketing.features.valuableContent.title",
    description:
      "contentMarketing.features.valuableContent.description",
  },
  {
    Icon: LinkRoundAngle,
    title: "contentMarketing.features.distribution.title",
    description:
      "contentMarketing.features.distribution.description",
  },
];

export const smmServices = [
  {
    Icon: Global,
    title: "smm.features.profileManagement.title",
    description: "smm.features.profileManagement.description",
  },
  {
    Icon: User,
    title: "smm.features.contentPlanning.title",
    description: "smm.features.contentPlanning.description",
  },
  {
    Icon: Chart,
    title: "smm.features.engagementAnalysis.title",
    description: "smm.features.engagementAnalysis.description",
  },
];

export const influencerMarketingServices = [
  {
    Icon: Volume,
    title: "influencerMarketing.features.collaborations.title",
    description:
      "influencerMarketing.features.collaborations.description",
  },
  {
    Icon: Target,
    title:
      "influencerMarketing.features.campaignTracking.title",
    description:
      "influencerMarketing.features.campaignTracking.description",
  },
];

export const targetedAdvertisingServices = [
  {
    Icon: Target,
    title: "targetedAdvertising.features.campaignSetup.title",
    description:
      "targetedAdvertising.features.campaignSetup.description",
  },
  {
    Icon: Global,
    title: "targetedAdvertising.features.adOptimization.title",
    description:
      "targetedAdvertising.features.adOptimization.description",
  },
];

export const ppcServices = [
  {
    Icon: Cursor,
    title: "ppc.features.payPerClick.title",
    description: "ppc.features.payPerClick.description",
  },
  {
    Icon: Graph,
    title: "ppc.features.campaignMonitoring.title",
    description: "ppc.features.campaignMonitoring.description",
  },
];

export const emailMarketingServices = [
  {
    Icon: Mailbox,
    title: "emailMarketing.features.audienceEngagement.title",
    description:
      "emailMarketing.features.audienceEngagement.description",
  },
  {
    Icon: Chart,
    title: "emailMarketing.features.automation.title",
    description:
      "emailMarketing.features.automation.description",
  },
];

export const analyticsAndAuditsServices = [
  {
    Icon: Chart,
    title:
      "analyticsAndAudits.features.performanceEvaluation.title",
    description:
      "analyticsAndAudits.features.performanceEvaluation.description",
  },
  {
    Icon: Global,
    title:
      "analyticsAndAudits.features.competitiveBenchmarking.title",
    description:
      "analyticsAndAudits.features.competitiveBenchmarking.description",
  },
];
